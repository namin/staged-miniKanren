  ...
  (lambda (tag pred)
    (lambda (u)
      (lambdag@ (c : S D A T C)
        (case-value (walk u S)
          ((x) (cond
                 ((make-tag-A+ x tag pred c S D A T C) =>
                  unit)
                 (else (mzero))))
          ((au du) (mzero))
          ((u) (cond
...
(define subsume-A+
  (lambda (x* S D A T C)
    (cond
      ((null? x*) `(,S ,D ,A ,T ,C))
      (else (let ((x (car x*)))
              (let ((D/T (update-D/T x S D A T C)))
...
(define =/= 
  (lambda (u v)
    (lambdag@ (c : S D A T C)
      (cond
        ((unify u v S) => (post-unify-=/= S D A T C))
        (else (unit c))))))
...
(define post-verify-A
  (lambda (S D T C)
    (lambda (A)
      (let ((D (subsume A D)))
        (cond
          ((verify-T T S) => (post-verify-T S D A C))
          (else #f))))))
...
(define post-verify-T
  (lambda (S D A C)
    (lambda (T)
      (subsume-T T S (subsume T D) A '() C))))

(define subsume-T  
  (lambda (T+ S D A T C)
    (let ((x* (rem-dups (map lhs A))))
      (subsume-T+ x* T+ S D A T C))))

(define subsume-T+ 
  (lambda (x* T+ S D A T C)
    (cond
      ((null? x*)
       (let ((T (append T+ T)))
         `(,S ,D ,A ,T ,C)))
       ...
       (let ((a (car A)))
         (cond
           ((and (eq? (lhs a) x)
              (or (tag=? (pr->tag a) 'sym)   
                  (tag=? (pr->tag a) 'num)))
            (update-D/T+ x '() S D T C))
           (else
	    (update-D/T x S D (cdr A) T C))))))))
  ...
  (lambda (tag u)
    (cond
      ...
      (else
       (lambdag@ (c : S D A T C)
         (cond
           ((absento+ u tag c S D A T C) => unit)
           (else (mzero))))))))

(define absento+  
  (lambda (u tag c S D A T C)
    (case-value (walk u S)
      ((x)
       (let ((T+ (ext-T x tag S T)))
         (cond
           ...
           (else
            (let ((D (subsume T+ D)))
              (subsume-T T+ S D A T C))))))
      ((au du)
       (let ((c (absento+ au tag c S D A T C)))
         (and c
           (let ((S (c->S c))
                 (D (c->D c))
                 (A (c->A c))
