...
;; This file records some weird behaviors of the fallback search that we discovered
;; while staging metaKanren, for purposes of future debugging.
    ...
    [(fresh (te1 te2 v1 v2 s c s1)
       ...
       (gather
        (conde
          [(== #f s1) (== '() $)]
          [(=/= #f s1) (== `((,s1 . ,c)) $)]))
       ;; WEIRDNESS: wrapping this in `later` leads to a more specialized answer. It doesn't
       ;; seem like it should, given that unifyo itself could just fallback when there is
...
(defrel/staged/fallback (eval-programo expr out)
  (fresh (lvar gexpr $ s/c*)
    ...
    (take-allo $ s/c*)
    ;; WEIRDNESS: when we change this to ==, it falls back the entire eval-programo rather than
    ;; the three calls within.
    (reifyo s/c* out)))
